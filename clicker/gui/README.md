# `clicker/gui` — Windows GUI (presentation layer)
...
```
┌─────────────────────────────────────────────────────────────┐
│  clicker/gui         ← you are here (presentation)         │
...
└────────────────────────┬────────────────────────────────────┘
                         ...
                         ▼
┌─────────────────────────────────────────────────────────────┐
│  clicker/runner/autopot, statusui, internal/*, platform/*   │
...
**`clicker/gui` must only import `clicker/runner` (the public facade).**
...
- ❌ `experimental-clicker/runner/internal/...`
- ❌ `experimental-clicker/runner/platform/...`
...
## What lives in each `clicker/gui` file
...
1. **Pure GUI change** (new tab, new widget, new layout): stay in
   `clicker/gui`. No new imports needed.
...
4. **Never** copy a detection threshold into a `clicker/gui` file as
...
grep -rn 'experimental-clicker/runner/' --include='*.go' clicker/gui/ \
  | grep -v 'experimental-clicker/runner"' \
  | grep -v 'experimental-clicker/runner/auto' \
  | grep -v 'experimental-clicker/runner/status' \
  | grep -v 'experimental-clicker/runner/internal' \
  | grep -v 'experimental-clicker/runner/platform'
```
