...
// maxConsecutiveFails is the number of consecutive OCR failures (any kind:
// panel not found, parse error, or heal failure) after which runStatusUI
// returns an error to trigger the pixel-bar fallback. Set to 1 so the
// fallback is immediate — any single issue with the status UI switches to
// the pixel-bar reader.
// Also used by healUntilStatusUI as its escape hatch.
		...
		if poller.NeedsValidation() {
			if err := a.validateWithLog(poller, cfg.Log); err != nil {
				...
				if consecutiveFails >= maxConsecutiveFails {
					return fmt.Errorf("status panel not found: %v", err)
				}
