// Package hpspreader reads Ragnarok Online HP/SP values from an
...
func NewReader(templatesDir string) (*Reader, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("hpspreader: glob %q: %w", pattern, err)
	}
	if len(files) == 0 {
		return nil, fmt.Errorf("hpspreader: no PNGs in %q", templatesDir)
	}
	...
	for _, fp := range files {
		...
		if err != nil {
			return nil, fmt.Errorf("hpspreader: read template %q: %w", fp, err)
		}
		mask := binarize(img)
	...
	if len(entries) == 0 {
		return nil, fmt.Errorf("hpspreader: no recognizable templates in %q (need 0-9, dot, slash, pipe, H, P, S)", templatesDir)
	}
...
func (r *Reader) Read(strip image.Image) Result {
	...
	//   1) binarize (raw, no morphology)
	//   2) 8-connected flood fill in findComponents (via
	//               floodFillCC below) bridges diagonal strokes
	//               that 4-conn splits (e.g. '7' crossbar + stem).
	//
	// dilate1 was tried first (1 px N4 dilation post-binarize)
	// but it regressed the synthetic round-trip: font9's
	// 1-px column gaps between adjacent glyphs closed up under
	// dilation, and adjacent digits collapsed into one 15×8
	// merged CC that scored 0 against every template. So
	// dilate1 is shipped but DISABLED by default and reachable
	// only via an explicit Reader option should we later find a
	// need that 8-conn alone cannot satisfy.
	...
	for i, cLocal := range comps {
		...
		ch, score := matchGlyph(glyph, templates)
		rect := image.Rect(
...
// dilate1 (kept in file as a documented helper; NOT wired into
// the default Read pipeline in this revision).
//
// Experimented with calling dilate1 from Read after binarize
// to bridge 1-px horizontal/vertical anti-alias gaps inside a
// glyph BEFORE flood fill. Result: regression on the synthetic
// integration test (TestReader_IntegrationSynthetic's font9
// glyphs have 1-px column gaps between them; dilation closed
// every gap and adjacent digits collapsed into a single 15×8
// CC, scoring 0 against every digit template and producing
// low_glyph_score).
//
// Take-away: 1-px dilation is over-aggressive at the inter-glyph
// scale; the fix that does not regress synthetic is 8-connectivity
// in floodFillCC alone, which bridges anti-alias-diagonal
// strokes (crossbar + diagonal stem of '7', '/' diagonals) WITHOUT
// affecting 1-px column gaps between distinct glyphs.
//
// If a future real-game capture finds a CC fragment that 8-conn
// does not bridge, the safe alternative is:
//   - Capture proper runtime-sized templates that match the
//     real-game glyph shapes, so partial-digit fragments have a
//     template to match against within the size-ratio envelope;
//     OR
//   - Run dilate1 only on the 5×3-or-smaller fragments
//     (size-gated dilation), not on the whole mask. That's a
//     contextual-morphology heuristic not currently shipped.
//
// (Body retained to make the experiment reproducible for
// future re-evaluation; dead code is intentionally NOT
// deleted, see the per-call comment for rationale.)
func dilate1(mask [][]bool) [][]bool {
	if len(mask) == 0 {
		return nil
	}
	h := len(mask)
...
func parseText(text string) (hp, hpMax, sp, spMax int, err error) {
	...
	if match == nil {
		err = errors.New("hpspreader: regex did not match")
		return
	}
